...
const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <button
        type="button"
        className={`w-full bg-white border rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm relative text-slate-900 transition-all duration-200 ${
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full bg-white shadow-xl max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm animate-in fade-in zoom-in-95 duration-100 origin-top">
          {options.map((option) => (
            <div
              key={option.value}
              className={`cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-50 text-slate-900 transition-colors ${
                ...
                {option.icon && (
                  <option.icon
                    className={`flex-shrink-0 h-4 w-4 mr-2 ${
                      value === option.value ? 'text-blue-500' : 'text-slate-400'
                    }`}
                  />
                )}
